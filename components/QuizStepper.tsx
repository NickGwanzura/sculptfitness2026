...
const QuizStepper: React.FC<{ onComplete: (data: QuizData) => void }> = ({ onComplete }) => {
  ...
  const SelectionCard = ({ selected, onClick, label, icon: Icon, description }: any) => (
    ...
    >
      ...
      <h4 className={cn("text-xs tracking-[0.15em] uppercase font-bold mb-1", selected ? "text-dark-text" : "text-dark-secondary/60")}>{label}</h4>
      {description && <p className="text-xs leading-relaxed text-dark-secondary/50 font-light">{description}</p>}
    </button>
  );
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12 md:py-20 min-h-[750px] flex flex-col justify-between">
